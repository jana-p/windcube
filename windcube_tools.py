...
def wind_fit(AllW, sProp, sDate):
    ...
    for VADscan in cl.ScanID['VAD']:
        ...
        if len(w.scan_ID)>0:
            ...
            # combine VAD at 15 and 75 degrees elevation
            if elestr=='15' or elestr=='75':
                wind['ele'] = int( elestr )
                wind['alt'] = [x[1] * np.sin( np.radians( float(elestr) ) ) for x in wind.index]
                combodf.append(wind)
    combodf = pd.concat(combodf)
    combodf = combodf[ ( (combodf['ele']==15) & (combodf.index.get_level_values('range')<200) ) | ( (combodf['ele']==75) & (combodf.index.get_level_values('range')>=200) ) ]

    combodf = combodf.reset_index().rename(columns={'range': 'oldrange', 'alt': 'range'})
